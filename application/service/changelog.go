...
func (s *ChangeLogService) ListProposalLogsGrouped(ctx context.Context, req *dto.ListProposalLogsGroupedReq) (*dto.ListProposalLogsGroupedResp, error) {
	...
	for _, proposal := range proposals {
		// 查询创建者信息
		creator, err := s.UserRepo.FindByID(ctx, proposal.UserID)
		if err != nil {
			logs.CtxWarnf(ctx, "[UserRepo] [FindByID] error: %v, userId: %s", err, proposal.UserID)
		}
		...
		if adminLog, ok := adminActionMap[proposal.ID]; ok {
			admin, err := s.UserRepo.FindByID(ctx, adminLog.UserID)
			if err != nil {
				logs.CtxWarnf(ctx, "[UserRepo] [FindByID] error: %v, userId: %s", err, adminLog.UserID)
			}
...
func (s *ChangeLogService) ListProposalLogsTimeline(ctx context.Context, req *dto.ListProposalLogsTimelineReq) (*dto.ListProposalLogsTimelineResp, error) {
	...
	for _, log := range logList {
		// 查询操作者信息
		operator, err := s.UserRepo.FindByID(ctx, log.UserID)
		if err != nil {
			logs.CtxWarnf(ctx, "[UserRepo] [FindByID] error: %v, userId: %s", err, log.UserID)
		}
