...
- 撤回课程涉及的教师若仍被任一未删除课程或提案通过 `teacherId` 引用则保留；两类引用均不存在时在同一事务中删除教师，并在提交后清理教师缓存。课程所用分类若仍被任一未删除课程或提案引用则保留，否则删除；课程所用院系还会检查教师所属院系引用，避免留下无效院系编号。映射删除后在事务提交后统一刷新缓存。首页综合搜索中的教师、院系和分类仅展示仍被未删除课程引用的值。
