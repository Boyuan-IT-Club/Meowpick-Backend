...
- 新版后端只保证部署后的撤销操作联动删除评论。旧版升级时需要检查历史上“课程已删除或不存在、评论仍未删除”的记录；若存在，应通过专门的数据修复同时软删除评论并删除对应点赞，不能在应用启动时静默修改历史业务数据。

旧版迁移已完成，一次性迁移工具已从当前代码移除。旧迁移步骤和冲突检查逻辑可从 Git 历史追溯。
